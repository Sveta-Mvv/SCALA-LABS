object Typeclasses extends App {
  ...
  println(testReversableString("oil"))
  ...
  def testSmashInt(a: Int, b: Int): Int = Smash.smash(a, b)
    println(testSmashInt(87, 32))

  // примените тайп-класс-решение из пункта (d) здесь
  def testSmashDouble(a: Double, b: Double): Double = Smash.smash(a, b)
    println(testSmashDouble(20.0, 7.0))


  // e) Реализуйте функцию Smash для типа String. Необходимо выполнить конкатенацию (склеивание) строк, которые будут получены в качестве параметра.
  ...
  println(testSmashString("happy new ", "year"))
}
